          ...
          {message.parts?.map((part, index) => {
            ...
            if (type === "tool-skillManager") {
              ...
              if (state === "output-available" && part.output) {
                ...
                // Skills list
                if ("skills" in output && Array.isArray(output.skills)) {
                  return (
                    <div className="w-full" key={toolCallId}>
                ...
                // Skill documentation or other output
                return (
                  <div className="w-full" key={toolCallId}>
                    <div className="rounded-2xl bg-gradient-to-br from-violet-900 via-slate-900 to-slate-900 p-4 shadow-lg">
...
export const PreviewMessage = memo(
  PurePreviewMessage,
  (prevProps, nextProps) => {
    // While the message is streaming, the underlying SDK may mutate parts in place.
    // If we memoize during streaming, we can miss incremental token updates and only
    // re-render once `isLoading` flips to false at the end.
